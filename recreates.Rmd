...
pilot_data <- pilot_data_OG %>% 
  ...
  mutate(
    ...
    color = case_when(
      program_type == "after" & item_value %in% c(30,38) ~ STEELBLUE,
      # program_type == "before" & item_value != 40 ~ LIGHTGRAY,
      program_type == "after" & item_value %in% c(12,6,14) ~ LIGHTSTEELBLUE,
      program_type == "before" & item_value == 40 ~ GRAY,
      program_type == "before" & item_value != 40 ~ LIGHTGRAY,
      # item_value == 30 ~ STEELBLUE,
      TRUE ~ GRAY
    )
  )
...
```{r}
# ISSUES WITH BARS SWITCHING POSITIONS

# ggplot(pilot_data, aes(x = item_type, y = item_value, fill = color)) +
#   scale_fill_identity(guide = "none") +
#   geom_col(position = "dodge") +
#   scale_y_continuous(limits = c(0,50)) +
#   geom_text(aes(label = item_value), nudge_x = -.25, nudge_y = -1, data = pilot_data %>% filter(program_type == "before")) +
#   my_swd_theme() + theme(
...
pilot_left_grob = grobTree(richtext_grob(
  "<span style=\"fontsize:5rem; color:GRAY\">BEFORE program, the<br><span style=\"color:ORANGE\">majority of children (40%)<br>felt just *OK*</span> about science.</span>",
  hjust = 0
))

pilot_right_grob = grobTree(richtext_grob(
  "<span style=\"fontsize:5rem\">AFTER program, <span style=\"color:STEELBLUE\">more children<br>were *Kind of interested (30%)*<br>& *Excited (38%)*</span> about science.</span>",
  hjust = 1
))
